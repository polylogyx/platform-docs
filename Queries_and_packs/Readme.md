...
After the PolyLogyx client is provisioned, seeded queries are run on the managed endpoints and the fetched data is stored in the database on the PolyLogyx server. You can query this data stored in [various tables](../08_Tables/Readme.md) to review node state and get real-time results.
