...
After the PolyLogyx client is provisioned, seeded queries are run on the managed endpoints and the fetched data is stored in the database on the PolyLogyx server. You can query this data (included in [various tables](../ 08_Tables/Readme.md).) to review node state and get real-time results.
You can review, push, and and manage queries from the UI or by using [APIs](../09_Rest_API/Readme.md). This topic describes how to manage queries using the UI. For more information on managing queries using API, see LINK TBA.
